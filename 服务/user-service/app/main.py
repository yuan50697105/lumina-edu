...
logger = logging.getLogger("lumina.user-service")
...
app = FastAPI(
    title="Lumina 墨光 · 用户服务 API",
    description="用户认证、资料与行为埋点服务",
...
async def api_logging_middleware(request: Request, call_next):
    """全量请求日志中间件：记录 method/path/status/耗时 到 api_logs"""
    with Timer() as timer:
        response = await call_next(request)

    # 异步中间件中需单独会话写入日志，不影响业务事务
    try:
        db = SessionLocal()
